...
public class ArticleMasterService extends IntentService {
	...
	protected void onHandleIntent(Intent intent) {
		...
		Integer commandNumber = myIntent
				.getIntegerAttribute(BuyStuffBundleConstants.COMMAND.getText());
		...
		try {
			...
			case LOAD_CATEGORIES:
				...
				if (categories != null) {
					bundle.putSerializable(
							BuyStuffBundleConstants.COMMAND.getText(),
			...
			case LOAD_SUBCATEGORIES:
				int id = myIntent
						.getIntegerAttribute(BuyStuffBundleConstants.SUBCAT_ID
								.getText());
				categories = fetchSubCategories(id);
				bundle.putSerializable(
						BuyStuffBundleConstants.SUBCATEGORIES.getText(),
						(Serializable) categories);
				break;
			case LOAD_ARTICLES_BY_SUBCATEGORY:
				int catId = myIntent
						.getIntegerAttribute(BuyStuffBundleConstants.CAT_ID
								.getText());
				...
				List<Article> articles = fetchArticlesBySubcategory(catId,
						subCatId);
				bundle.putSerializable(BuyStuffBundleConstants.ARTICLES.getText(),
						(Serializable) articles);
				break;
			case LOAD_ARTICLE:
				Log.v("Hola", "facu");
				int prodId = myIntent
						.getIntegerAttribute(BuyStuffBundleConstants.ARTICLE_ID
								.getText());
				Article p = fetchArticle(prodId);
				bundle.putSerializable(BuyStuffBundleConstants.ARTICLE.getText(),
						p);
				break;
			}
			...
			bundle.putString(BuyStuffBundleConstants.ERROR_MESSAGE.getText(),
					ioe.getMessage());
